...
const GooeyNav = ({
  ...
  return (
    <>
      {/* This effect is hard to recreate faithfully with Tailwind alone, so a style tag is used */}
      <style>
        {`
          .gooey-nav {
            --color-1: oklch(0.78 0.15 72);
            --color-2: oklch(0.6 0.19 42);
            --color-3: oklch(0.87 0.09 78);
            --color-4: oklch(0.95 0.01 80);
          ...
          .gooey-nav .point {
            ...
            opacity: 1;
            animation: gn-point calc(var(--time)) ease 1 -350ms;
          }
      ...
      <div className="gooey-nav relative" ref={containerRef}>
        <nav className="relative" style={{ transform: "translate3d(0,0,0.01px)" }}>
          <ul
            ref={navRef}
            className="flex list-none items-center gap-1 m-0 p-0"
          >
            {items.map((item, index) => (
              <li
                key={item.label}
                className={`${activeIndex === index ? "active" : ""} text-xs uppercase tracking-[0.14em]`}
